import React, { Component } from 'react';
import { Fade, Table } from "reactstrap";
import Service from '../../../components/Services/Service/Service'
import classes from './BillBuilderServices.css'

class BillBuilderServices extends Component{
    state = {
        fadeIn:true,
        currentButton: null,
        serviceButtons:null,
        servicesBuild: true
    };

    componentDidMount() {
        let addButtons = [
            <Fade in={this.state.fadeIn} tag="h5" className="mt-3">
                <button type="button" className={classes.deleteButton} onClick={this.deleteServiceHandler}><img src="data:image/png;base64,"
                                                                                                                alt="btn"/></button>
            </Fade>,
            <Fade in={this.state.fadeIn} tag="h5" className="mt-3">
                <button type="button" className={classes.addButton} onClick={this.serviceClick}><img
                    src="data:image/png;base64,"
                    alt="btn"/></button>
            </Fade>
        ];
        this.setState({serviceButtons: addButtons});
        this.setState({currentButton: addButtons[1]});
    }

    addService = (service) => {
        this.setState({fadeIn: !this.state.fadeIn});
        this.setState({currentButton: this.state.serviceButtons[0]});
        this.props.addService(service);
    };
    deleteService = (service) => {
        this.setState({fadeIn: !this.state.fadeIn});
        this.setState({currentButton: this.state.serviceButtons[1]});
        this.props.deleteService(service);
    };

    render(){
    return (
        <Table hover size='sm' className={classes.servicesTable}>
            <thead>
            <tr>
                <th>#</th>
                <th>Name</th>
                <th>Price</th>
                <th style={{textAlign: 'center'}}>Category</th>
            </tr>
            </thead>
            <tbody>
            {this.props.services.map((service, index) => {
                return (
                    <tr key={service.id} onClick={() => this.props.addService(service)}>
                        <Service
                            isBuild={this.props.servicesBuild}
                            currentButton={this.state.currentButton}
                            id={service.id}
                            serviceName={service.name}
                            price={service.price}
                            category={service.categoryName}
                            isModal={true}
                        />
                        <td>{this.state.currentButton}</td>
                    </tr>
                )
            })
            }
            </tbody>
        </Table>

    )}
}

export default BillBuilderServices;
